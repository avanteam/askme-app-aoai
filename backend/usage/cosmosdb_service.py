"""
CosmosDB Usage Tracking Service for token counting and usage analytics.
...
class CosmosUsageService(UsageProvider):
    """
    CosmosDB implementation of UsageProvider.
    ...
    def __init__(self, cosmos_client=None):
        ...
        self.logger.info(f"UsageTrackingService initialized - enabled: {self._enabled}, container: {self.container_name}")

    async def init_container(self):
        """Initialize the CosmosDB container for usage tracking."""
        if not self._enabled or not self.cosmos_client or not self.database_name:
    ...
    async def get_user_usage(
        ...
        """
        if not self._enabled or not self.container:
            await self.init_container()
            if not self.container:
                return []
...
# Ces fonctions seront remplacées par le factory dans usage_factory.py