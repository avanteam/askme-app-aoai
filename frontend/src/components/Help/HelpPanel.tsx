...
export function HelpPanel() {
  ...
  const filterPrompts = () => {
    ...
    if (searchQuery.trim() !== '') {
      ...
      filtered = filtered.filter(prompt => {
        ...
        return (
          title.includes(lowerCaseQuery) || description.includes(lowerCaseQuery) || promptText.includes(lowerCaseQuery)
        )
      })
    }
  ...
  const renderCategoryBadges = (categories: string[]) => {
    ...
    const categoryObjects = categories
      .map(categoryKey => helpContent.categories.find(cat => cat.key === categoryKey))
    ...
    return (
      <div className={styles.promptCard} onClick={() => copyPromptExample(item.prompt[currentLanguage])}>
        <div className={styles.promptCardHeader}>
          <Icon
            iconName={helpContent.categories.find(cat => cat.key === item.categories[0])?.icon || 'Tag'}
    ...
    return (
      <div className={`${styles.container} ${isVisible ? styles.visible : ''}`}>
        <div className={styles.helpHeader}>
          ...
          <button className={styles.closeButton} onClick={handleCloseHelp} aria-label="Fermer">
                ...
                <div className={styles.promptsList}>
                  {filteredPrompts.length === 0 ? (
                    <div className={styles.noResults}>
                      ...
                      <div className={styles.noResultsText}>{getTranslation('noPromptResults')}</div>
                    </div>
                  ) : (
                    <FocusZone>
                      <List items={filteredPrompts} onRenderCell={renderPromptItem} />
                    </FocusZone>
                  )}
                </div>
              </div>
